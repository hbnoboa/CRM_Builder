...
import { Link, useRouter } from '@/i18n/tenant-navigation';
...
// Mesmo componente do dashboard final (entity-dashboard): WidthProvider mede a largura
// real e o grid renderiza sem CSS scale, então o drag do RGL casa 1:1 com o cursor.
...
function TemplateBuilderContent() {
  ...
  const [headerCountEntitySlug, setHeaderCountEntitySlug] = useState('');
  ...
  if (!isCreateMode && template && !initialized) {
    ...
    setHeaderCountEntitySlug(((template as { settings?: { headerCountEntitySlug?: string } }).settings?.headerCountEntitySlug) || '');
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        settings: { headerCountEntitySlug: headerCountEntitySlug || undefined },
      };
      if (isCreateMode) {
            ...
            {(() => {
              ...
              <div>
                <ResponsiveGrid
                  ...
                  className="layout"
                  // Sem CSS scale: 12 colunas na largura natural do canvas (WidthProvider
                  // mede). breakpoint lg forcado (lg:0) p/ sempre desenhar as 12 colunas,
                  // independente da largura. Assim o arrasto do RGL casa 1:1 com o cursor.
        ...
        <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}>
          <DialogContent className="max-w-md max-h-[85vh] flex flex-col">
            ...
            <div className="space-y-4 overflow-y-auto flex-1 pr-1 -mr-1">
              ...
              <div className="space-y-1.5">
                <Label className="text-sm">Contar registros de</Label>
                <Select value={headerCountEntitySlug || '__default__'} onValueChange={(v) => setHeaderCountEntitySlug(v === '__default__' ? '' : v)}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="__default__">Padrão (entidade do dashboard)</SelectItem>
                    {allEntityList.map((e) => (
                      <SelectItem key={e.slug} value={e.slug}>{e.name}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                <p className="text-xs text-muted-foreground">Qual entidade o contador de registros do topo deve refletir</p>
              </div>
              <div className="space-y-1.5">
...
export default function DashboardTemplateBuilderPage() {
  return (
    <RequireRole module="templates">
      <DesktopOnlyEditor action="editar dashboards">
