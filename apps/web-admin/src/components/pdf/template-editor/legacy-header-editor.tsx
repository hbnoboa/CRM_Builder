...
interface LegacyHeaderEditorProps {
  header?: PdfHeader;
  onChange: (updates: Partial<PdfHeader>) => void;
  availableFields?: Array<{ slug: string; name: string; label?: string; type: string }>;
}

export function LegacyHeaderEditor({ header, onChange, availableFields = [] }: LegacyHeaderEditorProps) {
  ...
  if (header?.logo?.url && !header.logo.url.includes('{{') && !aspectRatio.current) {
    updateAspectRatio(header.logo.url);
  }

  const handleLogoChange = (updates: Partial<PdfHeader['logo']>) => {
    const newLogo = { ...header?.logo, ...updates } as PdfHeader['logo'];
    onChange({ logo: newLogo });
  };

  const handleTitleChange = (updates: Partial<PdfHeader['title']>) => {
    const newTitle = { ...header?.title, ...updates } as PdfHeader['title'];
    onChange({ title: newTitle });
  };

  const handleSubtitleChange = (updates: Partial<PdfHeader['subtitle']>) => {
    const newSubtitle = { ...header?.subtitle, ...updates } as PdfHeader['subtitle'];
    onChange({ subtitle: newSubtitle });
  };
  ...
  const hasLogo = !!header?.logo?.url;
  const isDynamicLogo = header?.logo?.url?.includes('{{');
            ...
            {isDynamicLogo ? (
              <div className="flex items-center gap-3 p-3 border rounded-lg bg-muted/50">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-mono truncate">{header?.logo?.url}</p>
          ...
          <div className="flex-1 space-y-1">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="w-10 text-xs text-muted-foreground text-right">
                {header?.title?.fontSize || 14}pt
              </span>
            </div>
          </div>
          <div className="flex items-center gap-2 pt-4">
            <Switch
              checked={header?.title?.bold || false}
        ...
        {availableFields.length > 0 && (
          <div className="space-y-1">
            ...
            <Select
              ...
              onValueChange={(value) => {
                if (value !== '_none') {
                  const currentText = header?.subtitle?.text || '';
