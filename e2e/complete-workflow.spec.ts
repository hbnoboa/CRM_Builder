...
test.describe.serial('CRUD - Data (Registros)', () => {
  test.beforeAll(async ({ request }) => {
    ...
    // Obter workspaceId e entitySlug
    const listResponse = await apiRequest(request, 'GET', '/entities', adminToken);
    const entities = await listResponse.json();
    const data = Array.isArray(entities) ? entities : entities.data || [];
    if (data.length > 0) {
  ...
  test('CREATE - deve criar registro na entidade', async ({ request }) => {
    if (!workspaceId || !entitySlug) {
      test.skip();
      return;
    }
    
    const response = await apiRequest(request, 'POST', `/data/${workspaceId}/${entitySlug}`, adminToken, TEST_DATA.record);
  ...
  test('READ - deve listar registros da entidade', async ({ request }) => {
    ...
    const response = await apiRequest(request, 'GET', `/data/${workspaceId}/${entitySlug}`, adminToken);
    expect(response.status()).toBe(200);
  ...
  test('READ - deve buscar registro por ID', async ({ request }) => {
    if (!workspaceId || !entitySlug || !createdRecordId) {
      test.skip();
      return;
    }
    
    const response = await apiRequest(request, 'GET', `/data/${workspaceId}/${entitySlug}/${createdRecordId}`, adminToken);
    expect(response.status()).toBe(200);
  });
  ...
  test('UPDATE - deve atualizar registro', async ({ request }) => {
    ...
    const response = await apiRequest(request, 'PUT', `/data/${workspaceId}/${entitySlug}/${createdRecordId}`, adminToken, updateData);
  ...
  test('DELETE - deve excluir registro', async ({ request }) => {
    ...
    const response = await apiRequest(request, 'DELETE', `/data/${workspaceId}/${entitySlug}/${createdRecordId}`, adminToken);
    expect(response.status()).toBeLessThan(300);
    createdRecordId = '';
  });
});
